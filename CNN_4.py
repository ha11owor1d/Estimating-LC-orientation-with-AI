...
# ─── Main ───────────────────────────────────────────────────────────────────
def main():
    for f in os.listdir("visualizations_4"):
        os.remove(os.path.join("visualizations_4", f)) if f.endswith(".png") else None
    ...
    test_ds  = PatchDirectorDataset(folder, patches_per_image=4,  random_crop=False, transform=val_transform)
    ...
    with torch.no_grad():
        ...
        save_dir = os.path.join("visualizations_4", "final")
        os.makedirs(save_dir, exist_ok=True)

        num_save = min(12, len(imgs))
